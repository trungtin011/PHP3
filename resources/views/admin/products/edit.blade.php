...
<div class="container mx-auto px-4 py-6 max-w-3xl">
    <h2 class="text-2xl font-semibold text-gray-800 mb-4 text-center">Sửa Sản Phẩm</h2>
    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" value="{{ old('slug', $product->slug) }}" placeholder="Tự động tạo nếu để trống">
            @error('slug')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            @if ($product->main_image)
                <img src="{{ Storage::url($product->main_image) }}" class="img-thumbnail mb-2" style="max-width: 150px;" alt="Ảnh chính hiện tại">
            @endif
            ...
            @error('main_image')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
            <img id="main_image_preview" class="img-thumbnail mt-2" style="max-width: 150px; display: none;" alt="Ảnh chính mới">
        </div>
            ...
            @if ($product->additional_images)
                <div class="d-flex flex-wrap gap-2 mb-2">
                    @foreach ($product->additional_images as $image)
                        <img src="{{ Storage::url($image) }}" class="img-thumbnail" style="max-width: 100px;" alt="Ảnh phụ">
                    @endforeach
                </div>
            @endif
        ...
        <div id="variants-section" class="mb-3">
            ...
            <div id="variant-rows">
                ...
                <div class="row mb-2">
                    <div class="col">
                        <input type="text" name="variants[{{ $index }}][name]" class="form-control" placeholder="Tên biến thể (e.g., Size)" value="{{ $variant->name }}">
                    </div>
                    <div class="col">
                        <input type="text" name="variants[{{ $index }}][value]" class="form-control" placeholder="Giá trị (e.g., Large)" value="{{ $variant->value }}">
                    </div>
                    <div class="col">
                        <input type="number" name="variants[{{ $index }}][price]" class="form-control" placeholder="Giá (tùy chọn)" value="{{ $variant->price }}">
                    </div>
                    <div class="col">
                        <input type="number" name="variants[{{ $index }}][stock]" class="form-control" placeholder="Tồn kho" value="{{ $variant->stock }}">
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="mb-3 flex gap-3 justify-end">
            <button type="submit" class="btn bg-orange-shopee text-white font-semibold hover:bg-orange-shopee-hover">
                <i class="fa-solid fa-save mr-2"></i> Cập nhật sản phẩm
            </button>
            <a href="{{ route('admin.products.index') }}" class="btn bg-gray-200 text-gray-700 font-semibold hover:bg-gray-300 border-orange-shopee">
